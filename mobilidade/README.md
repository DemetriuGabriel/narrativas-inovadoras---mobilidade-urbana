...
1.  **Narrativa Baseada em Markdown (`narrative.md`)**:
    Todo o conteúdo textual e a estrutura da história são gerados dinamicamente a partir de um único arquivo Markdown (`src/narrative.md`). Isso permite que escritores alterem a história diretamente sem precisar saber programar.
...
#### 1. O Parser de Narrativa (`narrativeParser.js`)

Este utilitário lê o arquivo `src/narrative.md` e o converte em componentes React. Ele suporta sintaxes especiais:
...
#### 2. Configuração da História (`storyConfig.js`)
...
5.  Abra `src/storyConfig.js` e adicione:
    ```javascript
    'novo-capitulo': {
